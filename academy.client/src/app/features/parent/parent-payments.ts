...
import { ParentApi, ParentChildDto, ParentPaymentItemDto } from '../../core/api/parent-api.service';
...
export class ParentPaymentsComponent implements OnInit {
  ...
  readonly items = signal<ParentPaymentItemDto[]>([]);
  ...
  load(): void {
    ...
    this.error.set(null);
    this.api.getPayments(this.childFilter(), this.page(), this.pageSize).subscribe({
      next: (data) => {
      ...
      error: (err) => {
        this.loading.set(false);
        this.error.set(err?.error?.detail || err?.message || 'Failed to load payments.');
      },
    });
  }
