...
namespace Academy.Application.Features.SuperAdmin.Billing.Queries.GetAdminGroupLedger;

public sealed record GetAdminGroupLedgerQuery(int LessonId, int GroupId)
    : IRequest<Result<IReadOnlyList<LedgerStudentRowDto>>>;

public sealed class GetAdminGroupLedgerQueryHandler(IApplicationDbContext dbContext)
    : IRequestHandler<GetAdminGroupLedgerQuery, Result<IReadOnlyList<LedgerStudentRowDto>>>
{
    public async Task<Result<IReadOnlyList<LedgerStudentRowDto>>> Handle(
        GetAdminGroupLedgerQuery request,
        CancellationToken cancellationToken)
    {
        var group = await dbContext.LessonGroups
            .AsNoTracking()
            .Where(x => x.Id == request.GroupId && x.LessonId == request.LessonId)
